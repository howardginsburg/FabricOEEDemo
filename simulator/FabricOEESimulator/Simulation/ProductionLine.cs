...
public sealed class ProductionLine
{
    ...
    // Raw material feed buffer (input to station 1)
    ...
    private void BuildStationChain(ILoggerFactory loggerFactory)
    {
        ...
        // Wire buffers between stations
        // Raw material → [Buffer] → Station1 → [Buffer] → Station2 → ... → StationN → (exit)
        ...
        // Last station's output goes to a "finished goods" buffer we drain
    ...
    {
        // Continuously feed raw materials (parts) into station 1's input buffer.
        // The bottleneck station's cycle time naturally limits throughput.
